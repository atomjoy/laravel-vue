...
export const useItemStore = defineStore('subscribers', () => {
	// State
	...
	async function loadList(search = '') {
		let res = await axios.get('/web/api/admin/subscribers?page=' + current_page.value + '&perpage=' + perpage.value + '&search=' + search);
	...
	async function deleteItem(id) {
		try {
			if (confirm('Delete ?')) {
				let res = await axios.delete('/web/api/admin/subscribers/' + id);
				setMessage(res);
	...
	async function createItem(e) {
		try {
			let res = await axios.post('/web/api/admin/subscribers', new FormData(e.target));
			setMessage(res);
			resetForm(e);
		} catch (err) {
			setError(err);
	...
	async function updateItem(id, data) {
		try {
			...
			let res = await axios.post('/web/api/admin/subscribers/' + id, data);
			setMessage(res);
		} catch (err) {
			setError(err);
		}
	...
	async function loadItem(id) {
		try {
			let res = await axios.get('/web/api/admin/subscribers/' + id);
			item.value = res?.data;
		} catch (err) {
			setError(err);
		}
	...
	async function removeImage(id) {
		try {
			let res = await axios.get('/web/api/admin/subscribers/remove/' + id);
			setMessage(res);
			loadItem(id);
		} catch (err) {
			setError(err);
