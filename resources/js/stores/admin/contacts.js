...
export const useItemStore = defineStore('contacts', () => {
	// State
	...
	async function deleteItem(id) {
		try {
			if (confirm('Delete ?')) {
				let res = await axios.delete('/web/api/admin/contacts/' + id);
				setMessage(res);
	...
	async function createItem(e) {
		try {
			let res = await axios.post('/web/api/admin/contacts', new FormData(e.target));
			setMessage(res);
			resetForm(e);
		} catch (err) {
			setError(err);
	...
	async function updateItem(id, data) {
		try {
			...
			let res = await axios.post('/web/api/admin/contacts/' + id, data);
			setMessage(res);
		} catch (err) {
			setError(err);
		}
	...
	async function loadItem(id) {
		try {
			let res = await axios.get('/web/api/admin/contacts/' + id);
			item.value = res?.data;
		} catch (err) {
			setError(err);
		}
	...
	async function removeImage(id) {
		try {
			let res = await axios.get('/web/api/admin/contacts/remove/' + id);
			setMessage(res);
			loadItem(id);
		} catch (err) {
			setError(err);
